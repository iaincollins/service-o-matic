    ...
    this.createService = function(serviceName, serviceObject, propertiesToExpose) {
        ...
        for (var property in serviceObject) {
            if (typeof serviceObject[property] == "function" || typeof serviceObject[property] == "object") {
                // The default is to only expose methods with "use webservice" declared in them
                if ( (propertiesToExpose == null && serviceObject[property].toString().match(/(\"|')use webservice(\"|')/))
                    // If propertiesToExpose is true then expose ALL properties
                    || propertiesToExpose === true
                    // Otherwise treat propertiesToExpose as whitelist array of properties to be exposed.
                    || (propertiesToExpose != null && propertiesToExpose.indexOf(property) > -1) ) {
                    ...
                    javascript += 'app.get("/' + webServiceName + '/' + webMethodName + '", function(req, res, next) {';
        ...
        app.get("/" + webServiceName, function(req, res, next) {
            ...
            html += "<h1>" + serviceName + " <small>as a Service</small></h1>";
            ...
            html += '</ul>';
            html += '<p class="muted">Generated by <a href="https://github.com/iaincollins/service-o-matic">Service-O-Matic</a></p>';
            html += '</div></div>';
            html += '</body>';
            res.send(html);
        });
    ...
    this.startServices = function(port) {
        ...
        if (port)
         ...
         app.get("/", function(req, res, next) {
             ...
             html += '<body class="container">';
             html += '<div class="row"><div class="col-sm-9">';
             html += "<h1>Service-O-Matic</small></h1>";
